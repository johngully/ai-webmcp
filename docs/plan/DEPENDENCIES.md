...
| `@testing-library/user-event` | Realistic form and management interactions | Phase 2 |
| `@testing-library/jest-dom` | Accessible DOM assertions | Phase 0 |
| `@playwright/test` | Browser-level manual and WebMCP flows | Phase 5 |
...
The TanStack CLI should scaffold the framework-owned packages first. Add the application-specific packages afterward:

```sh
pnpm add @tanstack/react-form zod nanoid @mcp-b/webmcp-polyfill
pnpm add -D prettier vitest @vitest/coverage-v8 jsdom @testing-library/react @testing-library/user-event @testing-library/jest-dom @playwright/test @mcp-b/webmcp-types
```

The scaffold is expected to supply React, TanStack Start, TanStack Router, TypeScript, Vite, the React Vite plugin, Nitro, and their type packages. Compare the generated manifest against the tables above before completing Phase 0.
