...
- [ ] **P4.1 — Initialize WebMCP on the client.** Load the polyfill only in the browser; allow it to stand down when native support exists. Completion criterion: `document.modelContext` is typed and available after hydration in the supported demo browser.
...
- [ ] **P4.6 — Surface WebMCP status.** Show a small demo status indicating registered, unavailable, or registration error without blocking the manual flow. Show the returned ID in an accessible in-page notification when this page executes the tool. Completion criterion: a presenter can confirm readiness before starting the WebMCP demonstration.
